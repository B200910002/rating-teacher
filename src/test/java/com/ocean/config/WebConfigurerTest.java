...
class WebConfigurerTest {
    ...
    void shouldCorsFilterOnApiPath() throws Exception {
        ...
        mockMvc
            .perform(
                options("/api/test-cors")
                    .header(HttpHeaders.ORIGIN, "http://localhost:3032")
                    .header(HttpHeaders.ACCESS_CONTROL_REQUEST_METHOD, "POST")
            )
            .andExpect(status().isOk())
            .andExpect(header().string(HttpHeaders.ACCESS_CONTROL_ALLOW_ORIGIN, "http://localhost:3032"))
        ...
        mockMvc
            .perform(get("/api/test-cors").header(HttpHeaders.ORIGIN, "http://localhost:3032"))
            .andExpect(status().isOk())
            .andExpect(header().string(HttpHeaders.ACCESS_CONTROL_ALLOW_ORIGIN, "http://localhost:3032"));
    }
    ...
    void shouldCorsFilterOnOtherPath() throws Exception {
        ...
        mockMvc
            .perform(get("/test/test-cors").header(HttpHeaders.ORIGIN, "http://localhost:3032"))
            .andExpect(status().isOk())
